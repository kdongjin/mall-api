...
public class JWTUtil {
	// 최소 256비트(32바이트) 40자리이상
	private static String key = "1234567890123456789012345678901234567890";

	/*
	 * 사용자 정보(Map)를 받아서 JWT 토큰을 생성 Map<String, Object> dataMap = new HashMap<>();
	 * dataMap.put("email", email); dataMap.put("pw", pw); dataMap.put("nickname",
	 * nickname); dataMap.put("social", social); dataMap.put("roleNames",
	 * roleNames); min : 토큰 유효한 시간(60분)
	 */
	public static String generateToken(Map<String, Object> valueMap, int min) {
		SecretKey key = null;
		try {
			// 문자열비밀키를 SecretKey객체변환(HMAC-SHA) HMAC(Hash-based Message Authentication Code)
